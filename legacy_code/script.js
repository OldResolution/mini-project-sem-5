...
// Load Cloropeth data
let PopwardLayer;
// Load Pop ward map data and store it in wardLayer
fetch('data/WardMap.geojson')
    .then(response => response.json())
    .then(geojsonData => {
...
async function populateHeatmap() {
    ...
    // Initialize the GeoJSON layer
    const CliwardLayer = L.geoJSON(geojsonData, {
        style: function (feature) {
            return {
                color: "#ffffff",
                weight: 1,
